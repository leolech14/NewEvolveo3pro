...
DEFAULT_CONFIDENCE_MAPPINGS: Dict[ExtractorType, callable] = {
    ExtractorType.PDFPLUMBER: lambda score: min(score * 0.9, 1.0),  # Conservative for layout changes
    ExtractorType.CAMELOT: lambda score: min(score * 0.85, 1.0),   # Table-specific reliability
...
class ConfidenceCalibrator:
    ...
    def train_calibration(
        ...
        if len(raw_scores) < 5:
            print(f"Not enough data to calibrate {extractor_type.value} (need ≥5, got {len(raw_scores)})")
            return
...
def calculate_extraction_confidence(
    ...
    pattern_quality = pattern_matches / max(transactions_found, 1) if transactions_found > 0 else 0
...
def merge_confidence_scores(scores: List[float], strategy: str = "weighted_average") -> float:
