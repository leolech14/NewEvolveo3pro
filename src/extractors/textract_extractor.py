...
class TextractExtractor(BaseExtractor):
    ...
    def extract(self, pdf_path: Path, s3_bucket: Optional[str] = None) -> PipelineResult:
        ...
        try:
            extraction_func = lambda: self._extract_with_textract(pdf_path, s3_bucket)
    ...
    def _extract_with_textract(self, pdf_path: Path, s3_bucket: Optional[str]) -> tuple[List[Transaction], Dict[str, Any], int]:
        ...
        raw_data = {
            ...
            "page_count": len([b for b in result["Blocks"] if b["BlockType"] == "PAGE"]),
            "total_blocks": len(result["Blocks"]),
            "table_count": len([b for b in result["Blocks"] if b["BlockType"] == "TABLE"]),
    ...
    def _wait_for_job_completion(self, job_id: str, max_wait_time: int = 300) -> Dict:
        ...
        while time.time() - start_time < max_wait_time:
            ...
            if status == 'SUCCEEDED':
                ...
                raise ExtractionError(f"Textract job failed: {response.get('StatusMessage', 'Unknown error')}")
    ...
    def _process_table_block(self, table_block: Dict, all_blocks: List[Dict]) -> List[Transaction]:
    ...
    def _parse_table_row(self, row_data: Dict[int, str]) -> Optional[Transaction]:
    ...
    def _parse_text_line(self, line: str) -> Optional[Transaction]:
