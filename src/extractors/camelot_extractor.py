...
class CamelotExtractor(BaseExtractor):
    ...
    def extract(self, pdf_path: Path) -> PipelineResult:
        ...
        try:
            extraction_func = lambda: self._extract_with_camelot(pdf_path)
    ...
    def _extract_with_camelot(self, pdf_path: Path) -> tuple[List[Transaction], Dict[str, Any], int]:
        ...
        raw_data = {
            "extractor": "camelot",
            "lattice_tables": len(lattice_tables) if 'lattice_tables' in locals() else 0,
    ...
    def _find_date_column(self, df) -> Optional[int]:
    ...
    def _find_amount_column(self, df) -> Optional[int]:
    ...
    def _find_description_column(self, df, date_col: Optional[int], amount_col: Optional[int]) -> Optional[int]:
    ...
    def _parse_table_row(
        ...
        try:
            ...
            transaction_type = TransactionType.INTERNATIONAL if self._looks_international(description) else TransactionType.DOMESTIC
            ...
            return Transaction(
                ...
                currency_orig="BRL",
                confidence_score=confidence * 0.9,  # Slightly lower confidence for table extraction
    ...
    def _calculate_confidence(self, transactions: List[Transaction], raw_data: Dict[str, Any]) -> float:
        ...
        table_count = raw_data.get("lattice_tables", 0) + raw_data.get("stream_tables", 0)
