...
from .base_extractor import BaseExtractor, ExtractionError, UnsupportedFormatError
...
class PdfplumberExtractor(BaseExtractor):
    ...
    def extract(self, pdf_path: Path) -> PipelineResult:
        ...
        try:
            extraction_func = lambda: self._extract_with_pdfplumber(pdf_path)
    ...
    def _extract_with_pdfplumber(self, pdf_path: Path) -> tuple[List[Transaction], Dict[str, Any], int]:
    ...
    def _try_national_pattern(self, line: str) -> Optional[Transaction]:
    ...
    def _try_fx_pattern(self, line: str) -> Optional[Transaction]:
    ...
    def _try_fallback_pattern(self, line: str) -> Optional[Transaction]:
    ...
    def _calculate_confidence(self, transactions: List[Transaction], raw_data: Dict[str, Any]) -> float:
        ...
        avg_transaction_confidence = sum(t.confidence_score for t in transactions) / len(transactions)
        ...
        # Combine scores
        confidence = 0.7 * avg_transaction_confidence + 0.3 * min(pattern_match_ratio, 1.0)
