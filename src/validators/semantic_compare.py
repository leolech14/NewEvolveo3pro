...
class SemanticComparator:
    ...
    def compare_transactions(
        ...
        for key in unmatched1:
            t = set1[key]
            mismatched_cells.append(f"Missing in second set: {t.date} {t.description} {t.amount_brl}")
        
        for key in unmatched2:
            t = set2[key]
            mismatched_cells.append(f"Extra in second set: {t.date} {t.description} {t.amount_brl}")
    ...
    def _find_matches(
        self,
        set1: Dict[tuple, Transaction],
        set2: Dict[tuple, Transaction]
        ...
        for key1 in remaining_keys1:
            ...
            for key2 in remaining_keys2:
                ...
                if score > best_score and score >= self.description_similarity_threshold:
                    best_score = score
                    best_match = key2
    ...
    def _calculate_similarity(self, t1: Transaction, t2: Transaction) -> float:
        ...
        amount_score = max(0, 1 - (float(amount_diff) / float(max_amount))) if max_amount > 0 else 1.0
    ...
    def _compare_transaction_fields(self, t1: Transaction, t2: Transaction) -> List[FieldComparison]:
        ...
        comparisons.append(FieldComparison(
            ...
            matches=desc_match,
            normalized_value1=self._normalize_description_for_comparison(t1.description),
            normalized_value2=self._normalize_description_for_comparison(t2.description),
    ...
    def _normalize_description_for_comparison(self, description: str) -> str:
        ...
        filler_words = {'de', 'da', 'do', 'em', 'na', 'no', 'a', 'o', 'e', 'para', 'com'}
