...
from ..core.models import Transaction, ValidationResult, ExtractorType
...
class GoldenValidator:
    ...
    def _load_golden_transactions(self) -> None:
        ...
        for golden_file in self.golden_dir.glob("*.csv"):
            try:
                ...
                print(f"Loaded {len(transactions)} golden transactions from {golden_file.name}")
    ...
    def validate_all_available(
        self,
        extraction_results: Dict[str, List[Transaction]]
    ...
    def add_golden_transactions(self, pdf_name: str, transactions: List[Transaction]) -> None:
    ...
    def create_golden_from_transactions(
        ...
        for t in transactions:
            data.append({
                ...
                'description': t.description,
                'amount_brl': f"{t.amount_brl:.2f}".replace('.', ','),  # Brazilian format
                'category': t.category or '',
            })
