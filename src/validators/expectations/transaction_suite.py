...
class TransactionExpectationSuite:
    ...
    def _create_suite(self) -> ExpectationSuite:
        ...
        expectations = [
            ...
            self._expect_column_value_lengths_to_be_between(
                "description", min_value=1, max_value=200
            ),
            self._expect_column_values_to_not_match_regex(
                "description", r"^\s*$"
            ),  # No empty strings
            # Amount validations
            ...
            self._expect_column_values_to_be_in_set(
                "category",
                [
                    "restaurant",
                    "supermarket",
            ...
            self._expect_column_values_to_be_between(
    ...
    def _expect_table_row_count_to_be_between(self) -> ExpectationConfiguration:
        ...
        return ExpectationConfiguration(
            ...
            meta={"notes": "Monthly statements should have 1-200 transactions"},
        )
    ...
    def _expect_table_columns_to_match_ordered_list(self) -> ExpectationConfiguration:
        ...
        return ExpectationConfiguration(
            ...
            meta={"notes": "Transaction CSV must have required columns in order"},
        )
    ...
    def _expect_column_values_to_not_be_null(
        ...
        return ExpectationConfiguration(
            ...
            meta={"notes": f"{column} is required for all transactions"},
        )

    def _expect_column_values_to_match_regex(
        self, column: str, regex: str
    ) -> ExpectationConfiguration:
    ...
    def _expect_column_values_to_be_in_set(
        ...
        return ExpectationConfiguration(
            ...
            meta={"notes": f"{column} values should be in predefined set"},
        )
    ...
    def _expect_column_values_to_be_between(
        ...
        return ExpectationConfiguration(
            ...
            meta={"notes": f"{column} should be between {min_value} and {max_value}"},
        )
    ...
    def validate_transactions(
        self, transactions: list[Transaction], pdf_name: str | None = None
    ) -> dict[str, Any]:
        """
        ...
        # Extract key metrics
        success_percent = (
            validation_result.statistics.get("successful_expectations", 0)
            / validation_result.statistics.get("evaluated_expectations", 1)
            * 100
        )
    ...
    def validate_against_golden(
        ...
        combined_results = {
            ...
            "overall_success": ge_results["success"]
            and (golden_results.is_valid if golden_results else True),
            "pdf_name": pdf_name,
            "validation_timestamp": pd.Timestamp.now().isoformat(),
        }
    ...
    def generate_data_docs(
        self, output_dir: Path = Path("data/artefacts/data_docs")
    ) -> Path:
