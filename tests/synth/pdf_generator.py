...
class ItauStatementGenerator:
    ...
    def generate_statement(
        ...
        # Create PDF
        pdf_path = output_path or Path(f"synthetic_statement_{statement_date.strftime('%Y-%m')}.pdf")
    ...
    def _create_pdf(
        ...
        pdf.cell(0, 5, 'Central de Relacionamento: 4004-1111 (capitais e regiões metropolitanas)', 0, 1)
...
def generate_ci_test_files(output_dir: Path = None) -> None:
    ...
    generator.create_golden_csv(
        transactions,
        output_dir / "ci_test_golden.csv"
    )
    
    print(f"Generated CI test files:")
