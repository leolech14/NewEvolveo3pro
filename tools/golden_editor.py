...
def style_dataframe_by_confidence(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in styled_cols:
        if col in df.columns:
            styler = styler.applymap(highlight_low_confidence, subset=[col])
...
def run_validation(
    transactions: list[Transaction], pdf_name: str
) -> dict[str, Any] | None:
...
def main():
    ...
    selected_pdf_name = st.sidebar.selectbox(
        "Select PDF to process:", pdf_names, index=0
    )

    selected_pdf_path = next(
        pdf for pdf in available_pdfs if pdf.name == selected_pdf_name
    )
    ...
    st.markdown(
        "**Contributing Pipelines:** " + ", ".join(result["contributing_pipelines"])
    )
    ...
    **Red-bordered cells** have low confidence (<85%). Review and edit as needed.
    ...
    if st.session_state.transactions_df is not None:
        ...
        with col1:
            ...
            # Style the DataFrame
            style_dataframe_by_confidence(display_df)
        ...
        if st.button("🔍 Validate Against Golden", type="secondary"):
            ...
            with st.spinner("Running validation..."):
                validation_result = run_validation(
                    transactions, st.session_state.selected_pdf
                )
        ...
        if st.button("💾 Save as Golden", type="primary"):
            ...
            try:
                ...
                golden_path = validator.create_golden_from_transactions(
                    st.session_state.selected_pdf, transactions
                )
    ...
    if st.session_state.validation_result:
        ...
        with col2:
            st.markdown(
                f"**Amount Difference:** R$ {result['amount_difference_brl']:.2f}"
            )
